  ...
  const handleConfirmDelete = async () => {
    if (selectedProduct) {
      try {
        ...
        // Si el error es específico sobre compras asociadas, mostrar mensaje más claro
        if (error instanceof Error && error.message.includes('compras asociadas')) {
          // Aquí podrías mostrar un toast o alert con el mensaje específico
          console.log('No se puede eliminar el producto. Verifique que no tenga compras asociadas.');
        }
      } finally {
        setIsDeleting(false);
      }
