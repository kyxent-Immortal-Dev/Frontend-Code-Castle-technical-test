...
export const ClientListComponent: React.FC = () => {
  ...
  const handleDeleteClient = async (id: number) => {
    try {
      ...
      if (success) {
        ...
        // Check if the error is about associated sales
        const client = clients.find(c => c.id === id);
        if (client?.sales && client.sales.length > 0) {
          showToast('No se puede eliminar el cliente porque tiene ventas asociadas. Solo se puede desactivar.', 'error');
        } else {
  ...
  // Helper function to check if a client can be deleted
  const canDeleteClient = (client: Client): boolean => {
    return !client.sales || client.sales.length === 0;
  };

  // Helper function to get delete button tooltip
  const getDeleteButtonTooltip = (client: Client): string => {
    if (client.sales && client.sales.length > 0) {
      return `No se puede eliminar - ${client.sales.length} venta(s) asociada(s). Solo se puede desactivar.`;
    }
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="block lg:hidden">
        {/* Info about deletion restrictions for mobile */}
        <div className="alert alert-info mb-4">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <div>
            <h4 className="font-bold">Restricciones de eliminación</h4>
            <p className="text-sm">
              Los clientes con ventas asociadas no se pueden eliminar. Solo se pueden desactivar.
            </p>
          </div>
        </div>
        ...
        {clients.length > 0 ? (
          <ViewMobileClients
            ...
            getDeleteButtonTooltip={getDeleteButtonTooltip}
          />
        ) : (
      ...
      <div className="hidden lg:block">
        ...
        <div className="alert alert-info mb-4">
          ...
          <div>
            <h4 className="font-bold">Información sobre eliminación de clientes</h4>
            <p className="text-sm">
              Los clientes con ventas asociadas no se pueden eliminar por integridad de datos. 
              Solo se pueden desactivar cambiando su estado. Los clientes sin ventas sí se pueden eliminar.
            </p>
          </div>
        </div>
